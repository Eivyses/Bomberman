...
public class Level {
  ...
  private static final Texture EXPLOSION_TEXTURE = new Texture("textures\\Explosion.png");
  private final Batch batch;
  private int[][] level;
  private int height;
  private int width;

  public Level(final Batch batch) {
    this.batch = batch;
  }
    ...
    for (int x = getWidth() - 1; x >= 0; x--) {
      for (int y = getHeight() - 1; y >= 0; y--) {
        ...
        switch (tile) {
          ...
          case LevelDto.EXPLOSION:
            {
              batch.draw(TERRAIN_TEXTURE, x * 32, (getHeight() - 1 - y) * 32);
              batch.draw(EXPLOSION_TEXTURE, x * 32, (getHeight() - 1 - y) * 32);
              break;
            }
          default:
            break;
  ...
  public void setLevel(final LevelDto levelDto) {
